...
<div class="rotating-gradient"></div>
      <div class="content">
        ...
        <v-card-title class="text-center">
          <div class="title-badge">Локация</div>
        </v-card-title>
        ...
        <v-select
          ...
          :loading="isChoice"
          @update:menu="isChoice = !isChoice"
          variant="solo"
          label=""
          style=""
          density="compact"
          :items="store.allCities"
          v-model="selectedCity" />

        <!-- Label  Second-->
        <div class="text-center mb-4">
          <v-card-text class="datetime-badge font-weight-bold">
            <span class="text-yellow-lighten-2">Зона</span>
            <span style="font-size: 15px; padding: 0 2px">&#127758;</span>
            {{ store.currentData.zone || "Loading..." }}</v-card-text
          >
        </div>
        ...
        <div class="tiles-grid">
          <v-card v-for="(metric, index) in metrics" :key="index" class="forecast-tile" :style="metric.style" elevation="16" hover>
            ...
            <v-icon v-else-if="metric.icon" large :color="metric.color" class="tile-icon" :class="[{ pulse: metric.pulse, rotate: metric.rotate }]">
              {{ metric.icon }}
            </v-icon>

            <v-img v-else :src="metric.img" alt="image" style="width: 90px" :class="[{}]"> </v-img>
...
// watch change city selector , and  add  data and refresh new currentData
watch(selectedCity, async newCity => {
  if (!newCity) return;

  store.selectCity(newCity);

  await Promise.all([store.refreshData("weather"), store.refreshData("water"), store.refreshData("moon")]);
});
...
const metrics = computed(() => {
  const base = [
    ...
    {
      label: "Координати",
      value: store.currentData?.gps?.lat?.toString().slice(0, 5) + " | " + store.currentData?.gps?.lng?.toString().slice(0, 5) || null,
