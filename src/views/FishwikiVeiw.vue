<template>
  <div>
    <v-container fluid  class="fill-height pa-2 mt-1 my-container "
    style="background-image: url(&quot;/images/bg/main5.png&quot;); background-size: cover; background-position: center"
    >
      <v-row align="center" justify="center">
            ...
            <v-expand-transition>
              <div v-show="show" class="">
                <v-expand-transition>
                  <div v-show="show">
                    <!-- Textarea -->
                    <v-textarea label="" :model-value="fish.info" bg-color="light-blue-darken-4" readonly style="height: 230px" />
                  </div>
                </v-expand-transition>
              </div>
            </v-expand-transition>
        ...
        <v-row align="start" class="d-none d-sm-flex">
          <v-col cols="4">
            <!--   image -->
            ...
            <!-- text -->
            <!-- <v-card class=" pa-6 mb-4" elevation="3" rounded="lg" max-width="40em">
              <div class="d-flex align-center ga-4">
                <v-avatar size="56">
                  <v-img src="/images/logo/fishWiki.png" />
                </v-avatar>

                <div>
                  <div class="text-h5 font-weight-bold   ">Fish Wiki</div>

                  <div class="text-body-2 text-medium-emphasis mt-1">
                    Каталог с популярните морски риби – обитаващи черноморският регион на България. 
                    <br/> Техните характеристики – вид, поведение, начин на хранене и риболовни техники. 
                    <br/> Създадена за бърза ориентация и практическа употреба, с възможно най-кратката и съществена информация.
                  </div>
                </div>
              </div>
            </v-card> -->

            <v-timeline align="start" density="comfortable" class="pt-1">
              <v-timeline-item v-for="(c, i) in introTimeline" :key="i" :dot-color="c.color" size="small">
                <template v-slot:icon>
                  <v-avatar :icon="c.icon" :color="c.color"></v-avatar>
                </template>

                <div>
                  <h4 :class="`text-${c.color}`">
                    {{ c.bg }}
                  </h4>
                  <div class="laptop-fix-display">{{ c.bgInfo }}</div>
                </div>
              </v-timeline-item>
            </v-timeline>
          </v-col>
          ...
          <!-- Window col -->
          <v-col cols="4" class="">
            <v-window v-model="fishModel" continuous crossfade>
              <v-window-item v-for="f in fishes" :key="f.id">
                <v-card rounded="10" flat class="mx-auto glass-card">
                  <!-- Image -->
                  <v-img height :src="f.image" lazy-src="/images/fish/bg0.png" cover class="laptom-fix-display-img">
                    <template v-slot:placeholder>
                      <div class="d-flex align-center justify-center fill-height"><v-progress-circular indeterminate /></div>
                    </template>
                  </v-img>
                  <!-- Title -->
                  <v-card-title class=""> {{ f.title }} </v-card-title>

                  <div v-show="!show" class="d-fles float-right px-4 mt-5">
                    <v-btn @click="prevFishDesktop" class="mr-8" size="30" icon="mdi-arrow-left" variant="elevated" color="error"></v-btn>

                    <v-btn @click="nextFishDesktop" :disabled="currentIndex >= fishes.length - 1" size="30" icon="mdi-arrow-right" variant="elevated" color="success"></v-btn>
                  </div>
                  ...
                  <div class="chip-grid mt-2 px-2">
                    <v-chip v-for="key in chipOrder" :key="key" @click="btnChip(key)" :variant="activeChip === key ? 'outlined' : 'elevated'" size="small" :color="chipMeta[key].color">
                      <v-icon size="10">{{ chipMeta[key].icon }}</v-icon> {{ chipMeta[key].bg }}
                    </v-chip>
                  </div>
                  <!-- Dynamic text -->
                  <v-card-text class="fancy-card-text-bg">
                    <template v-if="currentText === 'shortInfo'"> {{ f[currentText] || "Loading..." }} </template>
                    <template v-else>
                      <span :class="`text-${chipMeta?.[currentText]?.color}`"> {{ chipMeta?.[currentText]?.bg }}: </span> {{ f[currentText] || "Load" }}
                    </template>
                  </v-card-text>

                  <!-- Extra info -->
                  <v-card-text class="fancy-card-text-bg-info"> {{ f.info }} </v-card-text>
                </v-card>
              </v-window-item>
            </v-window>
          </v-col>
          ...
          <!-- Temeline col -->
          <v-col cols="4">
            <v-timeline align="start" side="end" density="default" class="float-right">
              <v-timeline-item v-for="(c, i) in chipMeta" :key="i" :dot-color="c.color" size="small">
                ...
                <template #opposite>
                  <div class="text-right pr-3">
                    ...
                    <div class="laptop-fix-display">
                      {{ c.bgInfo }}
                    </div>
                  </div>
                </template>

                <!-- ИКОНА (остава вдясно при align="start") -->
                <template #icon>
                  <v-avatar :color="c.color">
                    <v-icon :icon="c.icon" />
                  </v-avatar>
                </template>
              </v-timeline-item>
...
const chipMeta = {
  ...
  type: {
    bg: "Тип",
    bgInfo: "Поведение на рибата – пасажна или единична, начин на движение",
...
const introTimeline = [
  {
    bg: "Структура",
    bgInfo: "Информацията е подредена по категории – тип, хранене, поведение и техники",
    icon: "mdi-view-grid",
    color: "blue-darken-1"
  },
  {
    bg: "Визуализация",
    bgInfo: "Всеки вид включва изображение и кратко описание за лесно разпознаване",
    icon: "mdi-image",
    color: "indigo-darken-1"
  },
  {
    bg: "Навигация",
    bgInfo: "Интерактивни елементи за бързо превключване между характеристиките",
    icon: "mdi-cursor-pointer",
    color: "green-darken-1"
  },
  {
    bg: "Практика",
    bgInfo: "Полезна информация както за планиране, така и при реален риболов",
    icon: "mdi-fish",
    color: "teal-darken-1"
  },
  {
    bg: "Фокус",
    bgInfo: "Бърза ориентация, лесно сравнение и достъп до най-важното",
    icon: "mdi-target",
    color: "orange-darken-2"
  },
  {
    bg: "AI",
    bgInfo: "Генерирана информация и текст, chatGTP",
    icon: "mdi-copyright",
    color: "red-darken-2"
  }
];
...
const nextFishDesktop = () => {
  if (fishModel.value < fishes.value.length - 1) {
    fishModel.value++;
    currentText.value = "shortInfo";
    activeChip.value = null;
  }
...
const prevFishDesktop = () => {
  if (fishModel.value > 0) {
    fishModel.value--;
    currentText.value = "shortInfo";
    activeChip.value = null;
  }
...
.glass-card {
  ...
  box-shadow: 0 10px 30px rgba(206, 201, 201, 0.35);

  color: white;
  overflow: hidden;
  border: 1px solid rgb(145, 28, 47);
}
...
.fancy-card-text-bg-info {
  margin: 2px;
  text-align: center;
  border-radius: 12px;
  background: linear-gradient(175deg, #001f3f, #003f7f, #0074d9);
  color: #cce7ff;
  box-shadow:
    inset 0 0 15px rgba(0, 150, 255, 0.25),
    0 0 10px rgba(0, 100, 255, 0.4);
